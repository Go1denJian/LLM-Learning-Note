...
| 笔记 | 内容 | 关联度 |
|------|------|--------|
| [Word-Embedding-Math-and-Implementation.md](./Word-Embedding-Math-and-Implementation.md) | 词嵌入数学原理与实现 | ⭐⭐⭐ 前置知识 |
| [Entropy-CrossEntropy-KL-Explained.md](./Entropy-CrossEntropy-KL-Explained.md) | 熵、交叉熵与KL散度详解 | ⭐⭐⭐ 损失函数基础 |
| [LSTM-Deep-Dive.md](./LSTM-Deep-Dive.md) | LSTM深度解析 | ⭐⭐⭐ 后续学习 |
| [GRU-and-Seq2Seq.md](./GRU-and-Seq2Seq.md) | GRU与序列到序列模型 | ⭐⭐⭐ 后续学习 |
| [Transformer-Math-and-Implementation.md](./Transformer-Math-and-Implementation.md) | Transformer数学与实现 | ⭐⭐ 进阶架构 |

### 实践项目建议

**入门级**：
1. **字符级语言模型**：用RNN生成莎士比亚风格的文本
   - 输入：字符序列
   - 输出：下一个字符的预测
   - 参考：Karpathy的char-rnn
