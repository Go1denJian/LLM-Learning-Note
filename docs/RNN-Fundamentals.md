...
5. [梯度消失与训练优化](#5-梯度消失与训练优化)
6. [从数学到代码：RNN完整实现](#6-从数学到代码rnn完整实现)
7. [与其他模型的关系](#与其他模型的关系)
8. [扩展阅读与实现](#扩展阅读与实现)
9. [参考资源](#参考资源)
10. [附录：符号表](#附录符号表)
...
BPTT（Backpropagation Through Time）是RNN训练的核心算法。本节详细推导梯度计算过程。
...
$$
\frac{\partial \mathcal{L}_{\text{seq}}}{\partial h_t} = \underbrace{W_{hy}^T (\hat{y}_t - y_t)}_{\text{直接梯度}} + \underbrace{W_{hh}^T \cdot \text{diag}(1-\tanh^2(a_{t+1})) \cdot \frac{\partial \mathcal{L}_{\text{seq}}}{\partial h_{t+1}}}_{\text{传播梯度}}
$$
...
$$
\frac{\partial \mathcal{L}_{\text{seq}}}{\partial W_{xh}} = \sum_{t=1}^{T} \frac{\partial \mathcal{L}_{\text{seq}}}{\partial h_t} \odot (1 - \tanh^2(a_t)) \cdot x_t^T
$$
